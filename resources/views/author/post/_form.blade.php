                ...
                <div class="body">
                    ...
                    <div class="form-group m-t-15">
                        <input type="checkbox" id="publish" class="filled-in"
                               name="status" {{(isset($post) && $post->status) ? "checked" : ""}}>
                        ...
                        <div class="form-line {{$errors->has('categories') ? "focused error" : ""}}">
                            ...
                            <select name="categories[]" id="category" class="form-control show-tick"
                                ...
                                @foreach($categories as $category)
                                    <option
                                        value="{{$category->id}}" {{isset($post) && $post->categories->map(function($category): int  {return $category->id;})->contains($category->id) ? 'selected' : '' }}>{{$category->name}}</option>
                                @endforeach
                    ...
                    <div class="form-group form-float">
                        <div class="form-line {{$errors->has('tags') ? "focused error" : ""}}">
                            ...
                            <select name="tags[]" id="tag" class="form-control show-tick" data-live-search="true"
                                ...
                                @foreach($tags as $tag)
                                    <option value="{{ $tag->id }}"
                                        {{ isset($post) && $post->tags->map(function($tag): int  {return $tag->id;})->contains($tag->id) ? 'selected' : '' }}
                                    >{{ $tag->name }}</option>
                                @endforeach
                            </select>
                        </div>
